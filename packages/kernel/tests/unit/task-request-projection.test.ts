...
describe("reduceProjectState — task requests (SoT-041)", () => {
  it("collects member projects from every genesis, flagging self", () => {
    const state = reduceProjectState(
      [genesis(SELF, "memorize"), genesis(HUB, "memorize_hub")],
      SELF,
    );
    ...
    const state = reduceProjectState(
      [
        ...
        mkEvent(
          ...
          REQUEST.id,
          { requestId: REQUEST.id, taskId: "task_local_1" },
          SELF,
        ),
      ],
      SELF,
    ...
    const state = reduceProjectState(
      [
        ...
        mkEvent(
          ...
          REQUEST.id,
          { requestId: REQUEST.id, reason: "already shipped in #238" },
          SELF,
        ),
      ],
      SELF,
    ...
    const state = reduceProjectState(
      [
        ...
        mkEvent(
          "task.request-accepted",
          "taskreq_ghost",
          { requestId: "taskreq_ghost", taskId: "t" },
          SELF,
        ),
      ],
      SELF,
