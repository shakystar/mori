...
describe("projector", () => {
  ...
  it("includes only accepted decisions in recentDecisions, sorted by recency", () => {
    ...
    const index = buildMemoryIndex(state);
    expect(index.recentDecisions.map((d) => d.id)).toEqual(["dec_new", "dec_old"]);
  });
  ...
  it("bumps lastSeenAt on session.heartbeat without changing status", () => {
    ...
    expect(state.sessions["sess_1"]?.lastSeenAt).toBe("2026-04-20T00:10:00.000Z");
  });
  ...
  it("appends task.item-appended items in order and ignores unsafe payloads", () => {
    const itemEvent = (id: string, field: string, text: unknown, createdAt: string): DomainEvent =>
      makeEvent({
        id,
    ...
    const state = reduceProjectState([
      ...
      itemEvent("evt_r1", "riskNotes", "Blocked on upstream API key", "2026-04-12T02:00:00.000Z"),
      itemEvent(
        "evt_ac1",
        "acceptanceCriteria",
      ...
      itemEvent("evt_bad2", "riskNotes", { nested: "object" }, "2026-04-12T05:00:00.000Z"),
  ...
  it("caps topTasks and recentDecisions at their configured maximums", () => {
    ...
    const decisionEvents = Array.from({ length: MAX_RECENT_DECISIONS + 5 }, (_, i) =>
      decisionEvent(
        `dec_${i}`,
        "accepted",
        `2026-04-${String(10 + (i % 20)).padStart(2, "0")}T00:00:00.000Z`,
    ...
    const state = reduceProjectState([projectCreated(), ...taskEvents, ...decisionEvents]);
...
function workstreamEvent(id: string, status: "active" | "paused" | "closed"): DomainEvent {
  return makeEvent({
    id: `evt_${id}`,
    type: "workstream.created",
    scopeType: "workstream",
