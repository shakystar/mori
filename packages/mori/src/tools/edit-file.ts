...
export function editFile(
  ...
  if (!stats) {
    ...
    try {
      ...
      return { ok: false, reason: `failed to write file: ${error instanceof Error ? error.message : String(error)}` };
    }
  ...
  const updated = replaceAll ? content.split(oldString).join(newString) : replaceOnce(content, oldString, newString);
...
function atomicWrite(targetPath: string, content: string, mode?: number): void {
  const tempPath = join(dirname(targetPath), `.${basename(targetPath)}.${randomBytes(6).toString("hex")}.mori-tmp`);
  try {
...
const editFileParameters = Type.Object({
  ...
  replaceAll: Type.Optional(
    Type.Boolean({
      description: "Replace every occurrence of oldString instead of requiring exactly one match. Defaults to false.",
      default: false,
    }),
  ),
});

export function createEditFileTool(root: string = process.cwd()): AgentTool<typeof editFileParameters, EditFileResult> {
  return {
    name: "edit_file",
    label: "Edit File",
    description:
