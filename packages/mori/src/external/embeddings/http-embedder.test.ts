...
describe("HttpEmbedder adaptive batch splitting", () => {
  it("halves a rejected sub-batch until it fits, preserving input order", async () => {
    const calls: string[][] = [];
    const inputs = ["A", "B", "C", "D", "E", "F", "G", "H"].map((c) => c.repeat(1000));
    ...
    expect(calls.some((c) => c.reduce((s, t) => s + t.length, 0) > 4000)).toBe(true);
  });

  it("truncates and retries a single input the server keeps rejecting", async () => {
    const calls: string[][] = [];

    const out = await embedder(fakeServer(1500, calls)).embed(["Z".repeat(5000)]);
  ...
  it("sends a small batch as a single request (unchanged behavior)", async () => {
    ...
    const out = await embedder(fakeServer(1_000_000, calls)).embed(["hi", "there"]);
