...
| [Q2](#q2--거짓인-것은-얼마나-거짓인가)            | 기준선 시점 거짓 3건의 어긋난 인용 수(이동/신규 분리): `compare-and-append` 24/0=24 · `contradiction-repeat` 1/0=1 · `compaction-consolidation-boundary` 25/17=**42**(owner PR #339 리뷰 2라운드가 각주 2의 `dbfc7ab` 신규 카운트 과소산정(1→3)을 지적 — 25+17=42, 1라운드의 40이 아니다). **검산 25는 재현되지 않았다 — 24가 나왔다** (이유는 §Q2) |
...
| 2   | `compare-and-append-residue-adjudication.md`              | `db80dfc` | `344b3ed`(생성, #304) → `eff6763`(#311) → `c58d5c2`(#323)                                                                                           | `eff6763`·`c58d5c2` 둘 다                                                     | **거짓**               | **거짓** — `37a449d`(#338)가 12개 토큰을 추가로 이동(`:2693→:2713` 등)                                                                              |
| 3   | `contradiction-repeat-adjudication.md`                    | `db6ff67` | `42602ae`(생성, #292) → `76954a0`(#295, 코드+문서) → `eff6763`(#311) → `c58d5c2`(#323)                                                              | `eff6763`만[^1]                                                               | **거짓**               | **거짓** — `37a449d`(#338)가 18개 토큰을 추가로 이동                                                                                                |
| 4   | `line-citation-staleness-adjudication.md`                 | `4eeb794` | `a482562`(자기 생성 커밋, #307)                                                                                                                     | 없음                                                                          | **참**                 | **거짓 — `37a449d`(#338)가 8개 file:줄 토큰을 이동, 선언 이후 처음으로 거짓 전환**                                                                  |
| 5   | `compaction-consolidation-boundary.md`                    | `8e8782b` | `9920b1c`(생성, #273) → `ff60e32`(#281) → `dbfc7ab`(#288) → `db6ff67`(#293) → `23d9716`(#326) → `337f2cb`(#329) → `ebd55f1`(#333) → `f1d80ce`(#334) | `23d9716`·`337f2cb`·`ebd55f1`·`f1d80ce`(이동) + `ff60e32`·`dbfc7ab`(신규)[^2] | **거짓**               | **거짓** — `37a449d`(#338)가 8개 토큰을 추가로 이동                                                                                                 |
| 6   | `consolidate-cursor-commit-crash-residue-adjudication.md` | `c58d5c2` | `03abc0e`(자기 생성 커밋, #325)                                                                                                                     | 없음                                                                          | **참**                 | **거짓 — `37a449d`(#338)가 71개 토큰을 이동, 선언 이후 처음으로 거짓 전환**                                                                         |
| 7   | `log-unique-constraint-b-residue-adjudication.md`         | `60f8931` | `3b14890`(자기 생성 커밋, #313)                                                                                                                     | 없음                                                                          | **참**                 | **거짓 — `37a449d`(#338)가 31개 토큰을 이동, 선언 이후 처음으로 거짓 전환**                                                                         |
...
[^2]:
    **PR #339 리뷰(owner, Codex P2 동의)로 정정**: 최초 버전은 이 세 커밋을 뭉뚱그려 "컬럼 폭
    ...
    - **`dbfc7ab`(#288)**: **PR #339 리뷰 2라운드(owner, Codex P2 동의)로 재정정**: 1라운드는
      W8 행의 `memory-retrieval-service.ts:46-47` 1개만 신규로 셌으나 프로즈 문단(§2.2-b,
      `:237` 부근)에 심긴 신규 토큰을 놓쳤다. `git show --unified=0 dbfc7ab -- docs/compaction-consolidation-boundary.md`(§6 부록)로 다시 세면 위치 기준 **3개**다:
      프로즈 문단의 `memory-retrieval-service.ts:46-47`(1개) + W8 행의
      `memory-retrieval-service.ts:46-47, 152-163`(콤마 토큰 2개, §Q2의 계수 규칙대로
      토큰별로 센다). 다른 토큰은 `+`/`-` 개수가 동일해 컬럼 폭 변화다.
...
- `eff6763`(#311): **PR #339 리뷰 2라운드(owner, Codex P2 동의)로 근거 목록 정정** — 1라운드는
  이 목록에 `c58d5c2`의 토큰(`:56-58`·`:61-69`·`:85`)을 잘못 섞어 넣었고 `:77→:97`의 발생
  횟수도 4곳으로 셌다(실제 5곳). `git show --format="" --unified=0 eff6763 -- docs/compare-and-append-residue-adjudication.md`(§6 부록)의 6개 hunk를 발생 위치별로 다시 세면 **10개**다:
  `:55→:75`(3곳), `:59→:79`(1곳), `:77→:97`(5곳), `:77-84→:97-104`(1곳) — 소계는 그대로
  10이고 목록만 실측으로 교체했다. 같은 커밋이 `contradiction-repeat-adjudication.md`도
...
| `compaction-consolidation-boundary.md`       |   25 |   17 | **42** | 이동 25: `23d9716` 2(`:2400-2412`가 `:2252-2255`·`:2611-2627` 둘로 갈라짐, 신규값 둘 다 카운트) + `337f2cb` 11(커밋 메시지 자체가 "11건" — 직접 diff로도 재현됨) + `ebd55f1` 10(9건 갱신 + grep 블록 예시 출력 1건) + `f1d80ce` 2(`:2141,2466→:2239,2693`, 콤마 토큰 각각). 신규 17(**PR #339 리뷰 2라운드로 정정, 1라운드는 15**): `ff60e32` 14 + `dbfc7ab` 3(각주 2 — 프로즈 1 + W8행 콤마 토큰 2) |
...
| 6   | `consolidate-cursor-commit-crash-residue-adjudication.md` | **거짓**(오늘, `37a449d`) | **닫힘** — 착수 시점 참이었으나 #314(PR #338, 2026-08-06 11:47 UTC 머지)가 이 문서도 인용을 밀었다(71 insertions/71 deletions, §6 부록). `#328`이 지적한 별개의 판정 오류(코드 diff 무관, 서술 오류)는 여전히 열려 있다 |
| 7   | `log-unique-constraint-b-residue-adjudication.md`         | **거짓**(오늘, `37a449d`) | **닫힘** — 착수 시점 참이었으나 #314(PR #338)가 착지하며 거짓으로 넘어갔다 — 31 insertions/31 deletions(§6 부록)                                                                                                        |
...
| 3    | §Q3 권고((c) 단정 철회)를 `CONTRIBUTING.md`에 명문화 — 이후 추가되는 산정 문서는 전칭 기준선 대신 로컬 SHA를 쓴다                                                                                                                                                                                                                                                                                       | `CONTRIBUTING.md` | `.github/`는 안 건드려 #322(CI advisory, `.github/` 담당)와 교집합 0. **`#337`(#321)이 같은 파일에 "코드 주석의 크로스파일 줄 인용" 절을 이미 심었다** — 파일은 겹치지만 절이 달라 내용 충돌 없음(위 서술 참고). 다른 조각과는 교집합 0 | 선행 없음, 다른 조각과 병행 가능. `#337`과는 머지 순서만 문제이지 관례 충돌은 아니다 |
...
## 6. 부록 — 판정 근거 원출력 (§Q1 기준선 · §Q2 검산 · 오늘 재검증)
...
### `dbfc7ab` 원출력 — 각주 2(compaction-consolidation-boundary.md) 신규 카운트 재정정 근거

owner PR #339 리뷰 2라운드가 지적한 대로, 1라운드는 `dbfc7ab`의 신규 토큰을 1개로 셌으나
프로즈 문단에 심긴 것을 놓쳤다. 실제 명령과 출력:
...
새 토큰(위치 기준): 프로즈 문단(`:237` 근방)의 `memory-retrieval-service.ts:46-47` 1개 +
W8 행의 `memory-retrieval-service.ts:46-47, 152-163`(콤마 토큰 2개) = **3개**. 1라운드는 W8
행만 보고 그마저 1개로(콤마 분리 없이) 셌다.

### `eff6763`/`c58d5c2` 원출력 — §Q2 검산 근거 목록 정정

owner PR #339 리뷰 2라운드가 지적한 대로, §Q2 검산 절의 `eff6763` 토큰 목록에
`c58d5c2`의 토큰(`:56-58`·`:61-69`·`:85`)이 섞여 있었다. 두 커밋의 실제 diff:
...
`eff6763`의 실제 발생 위치별 토큰(6개 hunk 전수): `:55→:75`(3곳) · `:59→:79`(1곳) ·
`:77→:97`(5곳) · `:77-84→:97-104`(1곳) = **10개**. `c58d5c2`는 `:55→:75`(3) ·
`:56-58→:76-78`(1) · `:59→:79`(2) · `:61-69→:81-89`(1) · `:77→:97`(3) · `:85→:105`(4)
= **14개**(`:40`·`projection-store.ts:160-165`는 값이 안 바뀐 bystander라 제외).
...
compare-and-append(거짓, 12토큰 추가 이동) · contradiction-repeat(거짓, 18토큰 추가 이동) ·
**line-citation-staleness(참→거짓 전환, 8토큰 이동)** · compaction(거짓, 8토큰 추가 이동) ·
**consolidate-cursor-commit-crash(참→거짓 전환, 71토큰=71 insertions/71 deletions)** ·
**log-unique-constraint-b(참→거짓 전환, 31토큰=31 insertions/31 deletions)** ·
consolidate-evidence-binding(판정불가 — SHA 없음, 참고로 6토큰 이동 확인됨).
