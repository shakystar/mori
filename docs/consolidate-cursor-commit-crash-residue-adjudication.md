...
| 실패 모드 1 (마)                      | 토픽 `.md` 파일이 회수된다                                              | 예            | 아래 (마) — 4라운드에서 추가. b6(커밋 **뒤에** 던지는 갈래)에서만 낡는다                                                                                                              |
| 실패 모드 1 (b6 축 적용 판정)         | b6에서 (나-1)·(나-2)는 **적용되지 않고** (가)·(다)·(라)·(마)가 적용된다 | **아니다**    | 어느 자원이 낡는지를 가르는 판정이지 그 자체로 낡음을 주장하지 않는다. 낡는다고 주장된 축은 각각 자기 자리에서 3종 세트를 받았다                                                      |
...
1. **꼬리가 던지면 창이 이미 소비된 상태가 된다.** 오늘은 `:2858`이 던지면 커서가 안 움직여
   ...
   **낡는 대상이 b1~b5와 정반대다.** 이 갈래에서는 `writeAll`이 커밋됐으므로:

   - **(나-1)·(나-2)는 적용되지 않는다.** 투영 테이블 13개도 `search_fts`도 이 경계가 append한
     `memory.consolidated`를 담은 채로 커밋됐다. 낡은 것이 없으니 회수할 것도 없다.
   - **write-ahead 마커는 오히려 지워진다.** 마커를 지우는 `:727`이 커밋된 그 트랜잭션 안이므로,
     회수를 무장하는 비트가 정상적으로 해제된 상태다 — 마커가 **안 서는** b1과 대칭이고,
     마커가 **서서 남는** b2~b5와 반대다.
   ...
   - **대신 토픽 `.md` 파일이 낡는다 — 아래 (마)다.** `Promise.all`이 중간에 던지므로 imported
     규칙 일부의 `.md`만 새 내용이고 나머지는 직전 리빌드의 내용 그대로다.
   ...
   **발화 조건 — import된 규칙이 있는 프로젝트에서만 이 갈래가 열린다.**
   ...
   1건 이상이고, 열린 갈래가 실제로 무언가를 _낡히는_ 조건은 아래 (마) ①이 적은 대로 직전
   ...
   **(마) 토픽 `.md` 파일 축 — b6에서만 낡고, 상한은 (나-1)과 같은 「리빌드 1회」다.** 위 넷과
   달리 이 축은 테이블도 인덱스도 아닌 **디스크 산출물**이다. 절 머리 규정 ①의 어휘를 넓힌
   것이 이 축 때문이다.
   ...
   - **③ 상한: 커밋하는 리빌드 1회 — 종류 무관.** 단 그 1회가 언제 오는지에는 상한이 없고
     (새 활동에 종속), 위 미발화 조건대로 디스크가 회복된 뒤의 1회다.
   ...
   **정리 — 축마다 상한이 다르다.**
   ...
   | (마) 토픽 `.md` (b6 전용) | `getTopicFile(...)`의 imported 규칙 `.md` 파일       | `projection-store.ts:744` (`writeJson`)                    | **리빌드 1회, 종류 무관** (디스크 회복 뒤) |

   **갈래별로 어느 축이 적용되는지도 같지 않다.** b1~b5에는 (가)·(나-1)·(나-2)·(다)·(라)가
   적용되고 (마)는 적용되지 않는다(`:740`에 닿지 못한다). b6에는 정반대로
   (가)·(다)·(라)·(마)가 적용되고 (나-1)·(나-2)는 적용되지 않는다.

   즉 이 실패 모드의 회수는 "무조건 있다"도 아니고 하나의 상한으로 적을 수 있는 것도 아니다.
   **상한이 있는 축은 (나-1), (나-2)의 마커 갈래, (마) 셋뿐이고, 그 상한도 「리빌드 수 1회」이지
   「시간」이 아니다.**
...
| 3    | **R1 창 축의 메모리만** (가드 hold 축은 아니다 — 4라운드 정정) | 페이로드 확장(#314 트리거 발동) + 하네스 신뢰 + **가드 hold 경계에서 흡수하면 유실**                               | 아니오                                                                                                                   | **어려움** |
| 4    | 없음                                                           | 중복이 남는다                                                                                                      | 아니오                                                                                                                   | —          |
...
똑같이 남는다(§Q3 후보3 「닫는 것」의 4라운드 정정). 어느 쪽도 §Q2.6 표 전체를 비우지 않는다.
...
3. **후보1이 정상 동작에서 막는 것이 없다 — 다시 판정한 뒤에도 선다.** #310이 후보2·3을 죽인
   ...
   **판정: 근거 3은 선다.** 후보1이 새로 만드는 대가는 전부 **「늦음」**이고 — 그중 셋은 상한이
   없으며((가)·(다)·(나-2) b1), b6에서만 낡는 (마) 토픽 `.md`는 상한이 리빌드 1회다(4라운드에서
   더한 축) — **「못 읽음」은 상한이 리빌드 1회이며 후보1이 만들지 않는다.** 결론은 후보1로
   유지된다.
...
후보3 「닫는 것」의 4라운드 정정). 메모리 축에 한해 후보3이 덮는 것은 후보1이 덮는 것의
**부분집합**이다. 순위가 안 바뀌는 이유 셋 — 셋 다 이 크레딧에 기대지 않게 다시 썼다.

1. **덮는 크기가 갈린다.** 후보1은 창 축을 통째로 닫고(메모리·세그먼트·임베딩·FTS), 후보3은
   그중 메모리 축만 닫는다. 창 축은 꼬리 예외 하나로 열리고 폭이 초 단위다(§Q1.1·§Q1.3 (나)).
   가드 hold 축은 **둘 다 남긴다** — 후보1은 만들지도 고치지도 않아서(§Q3 후보1 「닫는 것」),
   후보3은 거기서 흡수하면 유실이라 안 흡수해서. 3라운드가 후보3 쪽에만 준 크레딧이 사라지므로
   이 항목은 후보1로 더 기운다.
2. **후보3이 남기는 것도 있다.** 창 축의 세그먼트 중복은 후보3의 사각지대에 그대로 남고
   ...
   상한이 리빌드 1회이고 **후보1 전후로 같으므로** 이 비교에 들어가지 않는다. b6에서만 낡는
   (마) 토픽 `.md`도 같은 이유로 「늦음」이고 상한이 리빌드 1회다. 후보3의 대가는
   **잘못된 흡수가 슬라이스를 건너뛰면 유실**이다(§Q3 후보3 대가 2 — #136이 닫은 손실). 대가의
   종류가 다르고 후자가 더 무겁다. **그리고 4라운드는 그 대가가 물리는 자리를 하나 더
   확인했다** — 가드 hold 경계가 정확히 그 모양이다.
...
**§Q4의 권고·순위는 유지된다 — 명시 판정.** 4라운드가 바꾼 것은 차선(후보3)의 크레딧을 깎은
것뿐이고, 그 방향은 1순위(후보1)를 강화하는 쪽이다. 권고는 **후보1(1b 형태)**, 차선은
**후보3**으로 그대로다. §Q5의 조각·완료 조건·인터리브 테스트도 손댈 자리가 없다.
